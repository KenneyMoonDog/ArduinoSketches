...
class BitmapHandler {
  ...
  public:
    ...
    void renderImage(Adafruit_ILI9341 screen, int screenX, int screenY){
      ...
      // check if file OK
      if (!this->fileOK) {
        Serial.println("renderImage:  File not ok!");
        return false;
      }
      else {
        Serial.println("renderImage:  File OK!");
      }

      // make sure screenX, screenY is on screen
      if((screenX < 0) || (screenX >= screen.width()) 
        || (screenY < 0) || (screenY >= screen.height())){
        Serial.println("renderImage screenX or screenY is off the screen!");
        return false;
      }
      else {
        Serial.println("renderImage: screenX and screenY are ON the screen!");
      }
...
void setup() {
  ...
  Serial.println("CHECKING FILENAME ARRAY:");

  for (byte counter = 0; counter < MAX_IMAGES; counter++){
...
void loop() {
  ...
  if (imageToShow<0 &&images[imageToShow] != "") {
