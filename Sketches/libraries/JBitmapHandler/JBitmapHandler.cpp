...
JBitmapHandler::JBitmapHandler(){
    ...
    void JBitmapHandler::loadBitmap(String filename){
    ...
    bool JBitmapHandler::renderImage(Adafruit_ILI9341* screen, int screenX, int screenY){

      Serial.println("renderImage Started");
      ...
      // check if file OK
      if (!this->fileOK) {
        Serial.print("renderImage failed fail check for file: ");
        Serial.println(this->bmpFilename);
        return false;
      }
      ...
      this->bmpFile = SD.open(this->bmpFilename, FILE_READ);
      if (!this->bmpFile) {
        Serial.print("renderImage failed to open file: ");
        Serial.println(this->bmpFilename);
        return false;
      }
      ...
      this->bmpFile.close();
      Serial.println("renderImage Complete");
